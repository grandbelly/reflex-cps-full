...
class ResearchAgent(BaseAgent):
    """데이터 수집 및 연구 에이전트 (Medium 글의 ResearchAgent 모델링)"""
    ...
    async def process(self, context: AgentContext) -> AgentContext:
        ...
        # 1. 센서 데이터 수집 (질문 유형에 따라 다른 데이터 조회)
        try:
            # 기본: 최신 센서 데이터 수집
            ...
            context.research_notes["sensor_data"] = f"센서 데이터 수집 실패: {e}"
...
class AnalysisAgent(BaseAgent):
    ...
    async def process(self, context: AgentContext) -> AgentContext:
        ...
        if context.research_notes:
            analysis_parts.append("🔍 **연구 결과 요약**:")
    ...
    def _detect_qc_violations(self, sensor_data: List[Dict], qc_data: List[Dict]) -> List[str]:
        """QC 규칙 위반 감지 (Medium 글의 이상 감지 로직 참조)"""
        violations = []
...
class ReviewAgent(BaseAgent):
    ...
    async def process(self, context: AgentContext) -> AgentContext:
        ...
        review_parts.append(f"🔍 **검토 결과**: {approval_status}")
...
class MultiAgentOrchestrator:
    """다중 에이전트 오케스트레이터 (Medium 글의 AgentWorkflow 모델링)"""
    ...
    async def process_query(self, query: str) -> str:
        """Medium 글의 워크플로우 구현: Research → Analysis → Review"""
